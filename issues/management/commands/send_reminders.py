...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        threshold = timezone.now() - timedelta(hours=1)  # 3 дня без изменений

        # Заявки, которые давно не обновлялись и ещё не завершены
        ...
        if not overdue_problems.exists():
            self.stdout.write("Нет зависших заявок.")
            return

        # Группируем заявки по ответственному сотруднику
        ...
        # Отправляем индивидуальные письма ответственным
        for email, problems in reminders.items():
            subject = f"Напоминание: у вас {len(problems)} зависших заявок"
            message_lines = [f"Здравствуйте!\n\n"]
            message_lines.append(f"Следующие заявки не обновлялись более 3 дней:\n\n")
            ...
            self.stdout.write(self.style.SUCCESS(f"Отправлено письмо на {email} о {len(problems)} заявках"))

        # Общее письмо, если остались заявки без ответственного
        if general_reminders:
            ...
            if staff_emails:
                ...
                message_lines.append("Следующие заявки не назначены и не обновлялись более 3 дней:\n\n")
